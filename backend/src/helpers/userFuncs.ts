...
async function getUserByIdWithRefResovled(userId: string) {
  ...
  if (
    ...
    (userData.accountSettings as DocumentReference)
  ) {
    console.log("resolving account settings");
    userData.accountSettings = await resolveDocRef(
      userData.accountSettings as DocumentReference,
    );
  } else {
    console.log(
      "Account settings is not a DocumentReference. Type:",
      typeof userData?.accountSettings,
    );
  }
  ...
  if (Array.isArray(userData?.OwnedBoards)) {
    userData.OwnedBoards = await Promise.all(
      userData.OwnedBoards.map((boardRef: DocumentReference) =>
        resolveDocRef(boardRef),
      ),
    );
  }
  ...
  if (Array.isArray(userData?.SharedBoards)) {
    userData.SharedBoards = await Promise.all(
      userData.SharedBoards.map((boardRef: DocumentReference) =>
        resolveDocRef(boardRef),
      ),
    );
  }
